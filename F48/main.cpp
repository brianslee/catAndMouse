...
int main()
{
    ...
    if (!image.loadFromFile(resourcePath() + "maze.png")) {
        return EXIT_FAILURE;
    }
    texture.loadFromImage(image);
    
    
    if (!tx2.loadFromFile(resourcePath() + "Triangle.png")) {
        return EXIT_FAILURE;
    }
    
    if (!marineTexture.loadFromFile(resourcePath() + "alien.png")) {
        return EXIT_FAILURE;
    }
    ...
    vector<projectile2>::const_iterator iter2;
    vector<projectile2> projectileArray2;
